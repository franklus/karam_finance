...
def execute(
    filters: frappe._dict | dict[str, Any] | None = None,
) -> tuple[
    list[dict[str, Any]],
    list[dict[str, Any]],
    None,
    ...
    apply_selected_view(selected_view, rows, footer_rows, period_list, income_totals)
...
def normalise_report_filters(
    filters: frappe._dict | dict[str, Any] | None,
) -> frappe._dict:
...
def get_report_periods(
    filters: frappe._dict, *, accumulated_values: bool | None = None
) -> list[Any]:
...
def get_allowed_cost_centers(filters: frappe._dict) -> list[str] | None:
...
def build_cost_centre_rows(
    ...
    for cost_center in cost_centres:
        ...
        for period in period_list:
            row[period.key] = values.get(period.key, 0.0)
        rows.append(row)
    ...
    for cost_center in reversed(cost_centres):
        ...
        for period in period_list:
            parent_row[period.key] += child_row.get(period.key, 0.0)
    return rows
...
def get_amounts_by_cost_centre(
    *,
    ...
    return pnlcc_data.get_amounts_by_cost_centre(
        company=company,
        from_date=from_date,
        to_date=to_date,
        finance_book=finance_book,
        ...
        restrict_cost_centers=restrict_cost_centers,
    )
...
def get_report_amounts(
    *,
    ...
    return pnlcc_data.get_report_amounts(
        company=company,
        periods=periods,
        finance_book=finance_book,
        include_default_fb=include_default_fb,
...
def get_total_by_root_type(
    *,
    ...
    return pnlcc_data.get_total_by_root_type(
        company=company,
        from_date=from_date,
        to_date=to_date,
        root_type=root_type,
