...
def _run_amount_query(
    *,
    ...
    base_expression = gl.debit - gl.credit
    account_expression = gl.debit_in_account_currency - gl.credit_in_account_currency
...
def _build_amount_select_fields(
    *,
    ...
    fields.append(Sum(cast(Any, base_expression)).as_("base_amount"))
    if needs_account_currency:
        fields.append(Sum(cast(Any, account_expression)).as_("account_amount"))
    return fields
...
def _apply_optional_filters(
    ...
    *,
    ...
    if restrict_cost_centers:
        query = query.where(gl.cost_center.isin(restrict_cost_centers))

    projects = parse_multiselect(project_filters)
    if projects:
