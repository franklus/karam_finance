...
def get_data(filters: frappe._dict) -> list[frappe._dict]:
...
def _build_asset_filters(filters: frappe._dict) -> dict[str, Any]:
...
def _select_schedule_rows(
    ...
    selected: dict[str, frappe._dict] = {}
...
def _get_booked_depreciation_counts(
    filters: frappe._dict,
    schedules: dict[str, frappe._dict],
    ...
    schedule_to_asset = {
        cstr(row.get("name")): asset for asset, row in schedules.items()
    }
    counts_by_schedule: dict[str, list[int]] = {}
    from_date = getdate(filters.get("from_date"))
    for row in query.run(as_dict=True):
        schedule_name = cstr(row.get("schedule"))
        if schedule_name not in schedule_to_asset:
            continue
        counts = counts_by_schedule.setdefault(schedule_name, [0, 0])
        schedule_date = getdate(row.get("schedule_date"))
        if schedule_date and from_date and schedule_date < from_date:
            counts[0] += 1
        counts[1] += 1

    result: dict[str, tuple[int, int]] = {}
    for asset, row in schedules.items():
        counts = counts_by_schedule.get(cstr(row.get("name")))
        result[asset] = (counts[0], counts[1]) if counts else (0, 0)
    return result
...
def _build_summary_rows(
    ...
    schedules: dict[str, frappe._dict],
    gl_totals: dict[str, frappe._dict],
...
def _resolve_finance_book(filters: frappe._dict) -> str | None:
