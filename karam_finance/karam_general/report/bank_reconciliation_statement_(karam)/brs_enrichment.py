...
def populate_missing_party_names(entries):
    ...
    for party_type, parties in parties_by_type.items():
        ...
        if title_field == "name":
            name_map.update({(party_type, party): party for party in parties})
            continue
        ...
        name_map.update(
            {(party_type, row.name): row.get(title_field) or row.name for row in rows}
        )
