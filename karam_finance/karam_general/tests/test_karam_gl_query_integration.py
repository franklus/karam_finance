...
class TestKaramGLQueryIntegration(IntegrationTestCase):
    @override
    def setUp(self) -> None:
        super().setUp()
    ...
    def test_restricted_user_cannot_read_denied_history_or_movements(self) -> None:
        ...
        for enrich in (True, False):
            with self.subTest(enrich=enrich):
                ...
                self.assertEqual(
                    sum((Decimal(str(row.debit)) for row in rows), Decimal(0)),
                    Decimal("100.0004"),
                )
        openings = query.get_flat_account_currency_openings(
            self._filters(categorize_by="Flat Chronological")
        )
        self.assertEqual(set(openings), {(self.allowed, "USD")})
        self.assertAlmostEqual(openings[(self.allowed, "USD")], 100.0001, places=7)
            ...
            with self.subTest(enrich=enrich):
                ...
                for row in rows:
                    ...
                    totals[key] = totals.get(key, Decimal(0)) + Decimal(
                        str(row.debit_in_account_currency)
                    )
    ...
    def test_period_query_keeps_both_boundary_dates(self) -> None:
        ...
        self.assertEqual(
            sum((Decimal(str(row.debit)) for row in allowed), Decimal(0)),
            Decimal("0.0003"),
        )

    def test_account_tree_expansion_preserves_selected_roots_and_leaves(self) -> None:
        start = (
            int(frappe.db.sql("select coalesce(max(rgt), 0) from tabAccount")[0][0])
            + 10
        )
        group = "Query Group " + self.token
        self._store(
            "Account", group, company=self.company, is_group=1, lft=start, rgt=start + 5
        )
        for name, offset in ((self.allowed, 1), (self.denied, 3), (self.eur, 6)):
            frappe.db.set_value(
                "Account", name, {"lft": start + offset, "rgt": start + offset + 1}
            )
        for path in (
            "karam_general.report.general_ledger_(karam)",
            "reporting_currency.report.general_ledger_(reporting_currency)",
        ):
            module = importlib.import_module("karam_finance." + path + ".gl_filters")
            with self.subTest(module=module.__name__):
                self.assertEqual(
                    module.get_accounts_with_children([self.allowed]), [self.allowed]
                )
                self.assertEqual(
                    set(module.get_accounts_with_children([group]) or []),
                    {group, self.allowed, self.denied},
                )
                self.assertEqual(
                    set(module.get_accounts_with_children([group, self.eur]) or []),
                    {group, self.allowed, self.denied, self.eur},
                )
