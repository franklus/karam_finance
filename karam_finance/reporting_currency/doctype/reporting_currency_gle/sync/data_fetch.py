...
def cleanup_orphaned_rc_gle_records() -> int:
    """Clean up RC GLE records that reference non-existent GL entries.

    This handles GL entries that were physically deleted from the database.
    Manual entries (manual_entry=1) are excluded as they have no gl_entry link.

    Returns:
        Number of orphaned RC GLE records deleted
    """
    # Delete RC GLE records whose GL entries no longer exist
    # Exclude manual entries which intentionally have no gl_entry link
    frappe.db.sql("""
		DELETE rc FROM `tabReporting Currency GLE` rc
    ...
    # Get actual row count from the DELETE operation
    orphan_count = frappe.db.sql("SELECT ROW_COUNT()")[0][0] or 0

    if orphan_count > 0:
        frappe.logger().info(
            f"Deleted {orphan_count} orphaned RC GLE records (GL entries removed)"
        )
