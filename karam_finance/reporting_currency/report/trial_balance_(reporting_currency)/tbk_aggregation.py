...
from .tbk_company_currency import COMPANY_CCY_VALUE_FIELDS, apply_company_balances
...
def apply_balances_to_accounts(
    ...
    for account in accounts:
        ...
        apply_company_balances(account, opening, period)
        account.update(
            {
                "opening_debit": decimal_amount(opening.get("opening_debit", 0)),
                "opening_credit": decimal_amount(opening.get("opening_credit", 0)),
                "debit": decimal_amount(period.get("debit", 0)),
                "credit": decimal_amount(period.get("credit", 0)),
            }
        )
        ...
        raw_account_currency_values = {
            ...
            "debit_in_account_currency": decimal_amount(
                period.get("debit_in_account_currency", 0)
            ),
            "credit_in_account_currency": decimal_amount(
        ...
        account.update(dict[str, Any].fromkeys(ACCOUNT_CCY_VALUE_FIELDS, 0))
...
def apply_gl_data_to_accounts(
    ...
    for account in accounts:
        ...
        account.update(
            {
                field: decimal_amount(data.get(field, 0))
                for field in (
                    "opening_debit",
                    "opening_credit",
                    "debit",
...
def finalize_account_currency_values(
    ...
    for account in accounts:
        ...
        if len(currencies) != 1:
            ...
            account.update(dict[str, Any].fromkeys(ACCOUNT_CCY_VALUE_FIELDS))
            continue
        ...
        account.update(
            {
                field: decimal_amount(raw_values.get(field, 0))
