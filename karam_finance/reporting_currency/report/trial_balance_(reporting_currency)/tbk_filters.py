...
def validate_filters(filters: Any) -> None:
    ...
    _validate_fiscal_year_dates(filters)


def _validate_fiscal_year_dates(filters: Any) -> None:
...
def validate_date_range_filters(filters: Any) -> None:
    ...
    if filters.get("show_unclosed_fy_pl_balances"):
        filters.year_start_date = None
        filters.year_end_date = None
        return
...
def _normalise_checkboxes(filters: Any) -> None:
    for key in (
        "ignore_fiscal_year",
        "show_zero_values",
