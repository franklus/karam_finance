...
def get_gl_entries(
    ...
    *,
    ...
    """
    currency_map = {"company_currency": filters["presentation_currency"]}
    ...
    gl = frappe.qb.DocType("Reporting Currency GLE")
    voucher_data = None
...
def _get_entries_with_compact_history(
    ...
    history = (
        query.select(
            ...
            *(
                Cast_(Sum(amount(gl, field)), "varchar").as_(field)
            ...
            Sum(
                cast(
                    Any,
                    Abs(amount(gl, "debit_in_account_currency"))
                    + Abs(amount(gl, "credit_in_account_currency")),
                )
    ...
    for row in history:
        for field in amount_fields:
            row[field] = decimal_amount(row[field])
...
def _select_fields(
    ...
    *,
    ...
    fields = [
        ...
        gl.gl_entry.as_("source_gl_entry"),
        gl.manual_entry,
        gl.reporting_doe,
        gl.is_cancelled,
        ...
        *(
        ...
        gl.is_opening,
        gl.creation,
        Cast_(amount(gl, "debit_in_company_currency"), "varchar").as_(
            "debit_in_company_currency"
        ),
        Cast_(amount(gl, "credit_in_company_currency"), "varchar").as_(
            "credit_in_company_currency"
        ),
        Cast_(amount(gl, "debit"), "varchar").as_("debit"),
        Cast_(amount(gl, "credit"), "varchar").as_("credit"),
        Cast_(amount(gl, "debit_in_account_currency"), "varchar").as_(
            "debit_in_account_currency"
        ),
        Cast_(amount(gl, "credit_in_account_currency"), "varchar").as_(
            "credit_in_account_currency"
        ),
    ]
    ...
    if filters.get("add_values_in_transaction_currency"):
        fields.extend(
            [
                Cast_(amount(gl, "debit_in_transaction_currency"), "varchar").as_(
                    "debit_in_transaction_currency"
                ),
                Cast_(amount(gl, "credit_in_transaction_currency"), "varchar").as_(
...
def _build_qb_entry_exclusions(filters: dict[str, Any], gl: Table) -> list[Term]:
    return [
        Coalesce(gl[field], 0) == 0
        for key, field in (
...
def _build_qb_conditions(
    ...
    *,
    ...
    if entry_type == "Reporting DOE":
        conditions.append(gl.reporting_doe == 1)
    elif entry_type == "Manual":
        conditions.append(gl.manual_entry == 1)
    elif entry_type == "Synced GL":
        conditions.extend(
            [Coalesce(gl.reporting_doe, 0) == 0, Coalesce(gl.manual_entry, 0) == 0]
        )
    conditions.extend(_build_qb_entry_exclusions(filters, gl))
    if filters.get("rc_entry"):
        conditions.append(gl.name == filters["rc_entry"])
    if filters.get("voucher_type"):
        conditions.append(gl.voucher_type == filters["voucher_type"])
...
def _build_qb_date_conditions(filters: dict[str, Any], gl: Table) -> list[Term]:
    ...
    conditions = [gl.posting_date <= filters.get("to_date")]
    if filters.get("disable_opening_balance_calculation"):
        conditions.append(gl.posting_date >= filters.get("from_date"))
    return conditions
...
def _build_qb_dimension_conditions(filters: dict[str, Any], gl: Table) -> list[Term]:
    ...
    for dimension in dimensions:
        ...
        if filters.get(fieldname) and not meta.has_field(fieldname):
            frappe.throw(
                _(
                    "Accounting dimension {0} is not synchronised to Reporting Currency GLE"
...
def _build_qb_flat_opening_conditions(filters: dict[str, Any], gl: Any) -> list[Any]:
    ...
    if not filters.get("show_opening_entries") and not filters.get(
        "_ignore_is_opening"
    ):
        boundary |= gl.is_opening == "Yes"
    return [boundary, gl.posting_date <= filters.get("to_date")]
...
def get_flat_account_currency_openings(
    ...
    rows = (
        ...
        .select(
            ...
            currency.as_("account_currency"),
            Cast_(
                Sum(amount(gl, "debit_in_account_currency"))
                - Sum(amount(gl, "credit_in_account_currency")),
                "varchar",
            ).as_("opening_balance"),
        )
